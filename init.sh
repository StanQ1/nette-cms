...
sudo docker exec -it php-container chmod -R 775 /application/temp /application/log

echo "Copying app configuration file"
sudo docker exec -it php-container cp /application/config.example.neon /application/config/config.neon
...
sudo docker exec -it php-container php vendor/bin/phinx migrate
